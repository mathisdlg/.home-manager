...
let
  cfg = config.services.desktop.components.wallpaper.hyprpaper;
...
in {
  options.services.desktop.components.wallpaper.hyprpaper = {
  ...
  config = mkIf cfg.enable {
    ...
    home.file.".config/hypr/scripts/wallpaper.sh" = {
      ...
      text = ''
        ...
        LAT="${cfg.latitude}"
        LON="${cfg.longitude}"
        WALL_DIR_BOTH="${cfg.wallpapers_dir.both}"
        WALL_DIR_DAY="${cfg.wallpapers_dir.day}"
        WALL_DIR_NIGHT="${cfg.wallpapers_dir.night}"
