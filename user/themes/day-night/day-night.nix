...
# (see ../../components/hypr/hyprpaper/hyprpaper.nix) to also drive every
# other theme-able app: GTK apps, Qt apps, the bar, wofi/wlogout, dunst, and
# hyprland's own border colors.
#
# Two ways this fires:
...
with lib;
let
  cfg = config.services.themes.dayNight;
  ...
  waybarEnabled = config.services.component.waybar.enable or false;
  hyprpanelEnabled = config.services.component.hypr.hyprpanel.enable or false;
  wofiEnabled = config.services.component.wofi.enable or false;
  wlogoutEnabled = config.services.component.wlogout.enable or false;
  dunstEnabled = config.services.component.notifications.dunst.enable or false;
  themesEnabled = config.services.themes.themes.enable or false;
in
{
  options.services.themes.dayNight = {
  ...
  config = mkIf cfg.enable {
    ...
    home.file = {
      ...
      ".config/waybar/colors-day.css".source = ../../components/waybar/config/colors-day.css;
      ".config/waybar/colors-night.css".source = ../../components/waybar/config/colors-night.css;

      ".config/wlogout/colors-day.css".source = ../../components/wlogout/config/colors-day.css;
      ".config/wlogout/colors-night.css".source = ../../components/wlogout/config/colors-night.css;

      # --- wofi: full day/night stylesheets (none existed before) --------
      ".config/wofi/style-day.css".source = ../../components/wofi/config/style-day.css;
      ".config/wofi/style-night.css".source = ../../components/wofi/config/style-night.css;

      # --- dunst: full dunstrc variants, swapped + service restarted -----
      # (generated below via home.file.text, not .source, since these need
      # config.services.browser.notifyCommand interpolated in)
    ...
    home.file.".config/hypr/scripts/theme-mode/apply-mode.sh" = {
      ...
      text = ''
        ...
        ${optionalString hyprpanelEnabled ''
